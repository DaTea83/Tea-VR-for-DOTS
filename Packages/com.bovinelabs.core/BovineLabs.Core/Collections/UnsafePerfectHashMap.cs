...
namespace BovineLabs.Core.Collections {
    ...
    public unsafe struct UnsafePerfectHashMap<TKey, TValue> : IDisposable
        ...
        [NativeDisableUnsafePtrRestriction] internal TKey* Keys;

        [NativeDisableUnsafePtrRestriction] internal TValue* Values;
        internal int Size;
        internal TValue NullValue;

        private readonly AllocatorManager.AllocatorHandle allocator;

        public UnsafePerfectHashMap(NativeArray<TKey> keys,
        ...
        public static UnsafePerfectHashMap<TKey, TValue>* Alloc(
            ...
            TValue nullValue,
            AllocatorManager.AllocatorHandle allocator) {
            var data = (UnsafePerfectHashMap<TKey, TValue>*)Memory.Unmanaged.Allocate(
                sizeof(UnsafePerfectHashMap<TKey, TValue>),
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static int IndexFor(TKey key, int size) { return key.GetHashCode() & (size - 1); }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private int IndexFor(TKey key) { return IndexFor(key, this.Size); }
