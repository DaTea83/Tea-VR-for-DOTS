...
namespace BovineLabs.Core.States {
    ...
    public static class StateAPI {
        public static byte Register<TState, TInstance, TSettings>(ref SystemState systemState,
            string stateName,
        ...
        public static void Register<TState, TInstance>(ref SystemState systemState,
            byte stateKey,
            bool queryDependency = true)
            where TState : unmanaged, IComponentData
            where TInstance : unmanaged, IComponentData {
            Check.Assume(!systemState.EntityManager.HasComponent<StateInstance>(systemState.SystemHandle),
