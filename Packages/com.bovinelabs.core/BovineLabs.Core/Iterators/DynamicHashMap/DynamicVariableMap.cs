...
namespace BovineLabs.Core.Iterators {
    ...
    public unsafe struct
        DynamicVariableMap<TKey, TValue, T, TC> : IEnumerable<DynamicVariableMap<TKey, TValue, T, TC>.KVC>
        where TKey : unmanaged, IEquatable<TKey>
        where TValue : unmanaged
        where T : unmanaged, IEquatable<T>
        where TC : unmanaged, IColumn<T> {
        ...
        [NativeDisableUnsafePtrRestriction] private DynamicVariableMapHelper<TKey, TValue, T, TC>* helper;
        ...
        public bool TryAdd(TKey key, TValue item, T column) {
            ...
            var idx = DynamicVariableMapHelper<TKey, TValue, T, TC>.TryAdd(this.buffer, ref this.helper, key, item,
                column);
            return idx != -1;
        }
