...
namespace BovineLabs.Core.Editor.ChangeFilterTracking {
    ...
    public partial struct ChangeFilterTrackingSystem : ISystem {
        ...
        internal static readonly SharedStatic<float> WarningLevel =
            SharedStatic<float>.GetOrCreate<WarningLevelContext>();
        ...
        public void OnCreate(ref SystemState state) {
            ...
            foreach (var typeInfo in TypeManager.AllTypes) {
                if (typeInfo.Category is not TypeManager.TypeCategory.ComponentData
                    and not TypeManager.TypeCategory.BufferData) {
                    continue;
                }
        ...
        private struct DidChangeJob : IJobChunk {
            [ReadOnly] public FakeDynamicComponentTypeHandle DynamicTypeHandle;
            ...
            public void Execute(in ArchetypeChunk chunk,
                ...
                var changed = chunk.DidChange(this.DynamicTypeHandle.TypeIndex,
                    ref this.DynamicTypeHandle.TypeLookupCache, this.LastSystemVersion);
        ...
        private struct ValidateChangeFilterJob : IJob {
            ...
            public void Execute() {
                ...
                if ((this.Index + 1) % ShortUpdateTime == 0) {
                    var total = mathex.sum(this.Result.GetSubArray((this.Index - ShortUpdateTime) + 1,
                        ShortUpdateTime));
                ...
                if (this.Index == this.Result.Length - 1) {
                    ...
                    if (!this.HasWarned.Value && averageChange > WarningLevel.Data) {
                        ...
                        this.Logger.LogWarning512(
                            $"{this.TypeName} DidChange triggered on average {percent}% of chunks per frame");
