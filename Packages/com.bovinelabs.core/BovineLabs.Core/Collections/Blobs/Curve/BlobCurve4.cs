...
namespace BovineLabs.Core.Collections {
    ...
    public struct BlobCurve4 : IBlobCurve<float4> {
        ...
        public static void Construct(
            ...
            if (hasOnlyOneKeyframe) {
                ...
                builder.Allocate(ref blobCurve.segments, 1)[0] =
                    new BlobCurveSegment4(key0X, key0Y, key0Z, key0W, key0X, key0Y, key0Z, key0W);
                ...
                for (int i = 0, j = 1; j < keyFrameCount; i = j++) {
                    ...
                    segBuilder[i] = new BlobCurveSegment4(xKeys[i], yKeys[i], zKeys[i], wKeys[i], xKeys[j], yKeys[j],
                        zKeys[j], wKeys[j]);
                }
        ...
        private static void InputCurveCheck(AnimationCurve curveX,
            AnimationCurve curveY,
            AnimationCurve curveZ,
            AnimationCurve curveW) {
            ...
            for (int i = 0, len = xKeys.Length; i < len; i++) {
                ...
                if (!Mathf.Approximately(kx.time, ky.time) || !Mathf.Approximately(kx.time, kz.time) ||
                    !Mathf.Approximately(kx.time, kw.time)) {
                    throw new ArgumentException(
                        $"Time not sync Key[{i}, X time={kx.time}, Y time={ky.time}], Z time={kz.time}], W time={kw.time}]");
                }
