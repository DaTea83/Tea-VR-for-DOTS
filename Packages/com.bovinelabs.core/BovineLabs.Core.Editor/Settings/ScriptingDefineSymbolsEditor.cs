...
namespace BovineLabs.Core.Editor.Settings {
    ...
    public static class ScriptingDefineSymbolsEditor {
        ...
        public static void ApplyDefinesToAll(IReadOnlyList<string> scriptingDefines,
            IReadOnlyList<string> removeDefines) {
            foreach (var target in GetInstalledNamedBuildTargets()) {
                ApplyDefines(target, scriptingDefines, removeDefines);
            }
        }

        private static void ApplyDefines(NamedBuildTarget target,
            IReadOnlyList<string> addDefines,
            IReadOnlyList<string> removeDefines) {
