...
namespace BovineLabs.Core.Collections {
    ...
    public struct UnsafeArray<T> : IDisposable, IEnumerable<T>, IEquatable<UnsafeArray<T>>
        where T : unmanaged {
        [NativeDisableUnsafePtrRestriction] private unsafe void* buffer;
        ...
        public unsafe T this[int index] {
            ...
            [WriteAccessRequired] set => UnsafeUtility.WriteArrayElement(this.buffer, index, value);
        }

        public static bool operator ==(UnsafeArray<T> left, UnsafeArray<T> right) { return left.Equals(right); }
        ...
        public unsafe void Dispose() {
            ...
            if (this.allocatorLabel == Allocator.Invalid) {
                throw new InvalidOperationException(
                    "The UnsafeArray can not be Disposed because it was not allocated with a valid allocator.");
            }
        ...
        public unsafe void* GetUnsafePtr() { return this.buffer; }
        ...
        IEnumerator<T> IEnumerable<T>.GetEnumerator() { return new Enumerator(ref this); }
        ...
        public override unsafe int GetHashCode() { return ((int)this.buffer * 397) ^ this.Length; }
        ...
        public static void Copy(T[] src, UnsafeArray<T> dst, int length) { CopySafe(src, 0, dst, 0, length); }

        public static void Copy(UnsafeArray<T> src, T[] dst, int length) { CopySafe(src, 0, dst, 0, length); }
        ...
        private static void CheckAllocateArguments(int length, Allocator allocator) {
            ...
            if (allocator >= Allocator.FirstUserIndex) {
                throw new ArgumentException("Use CollectionHelper.CreateUnsafeArray for custom allocator",
                    nameof(allocator));
            }
        ...
        private static void IsUnmanagedAndThrow() {
            if (!UnsafeUtility.IsUnmanaged<T>()) {
                throw new InvalidOperationException(
                    $"{typeof(T)} used in UnsafeArray<{typeof(T)}> must be unmanaged (contain no managed types).");
            }
        }

        private static unsafe void CopySafe(UnsafeArray<T> src,
            int srcIndex,
            UnsafeArray<T> dst,
            int dstIndex,
            int length) {
        ...
        private struct UnsafeArrayDispose {
            [NativeDisableUnsafePtrRestriction] internal unsafe void* Buffer;

            internal Allocator AllocatorLabel;

            public unsafe void Dispose() { UnsafeUtility.FreeTracked(this.Buffer, this.AllocatorLabel); }
        }
