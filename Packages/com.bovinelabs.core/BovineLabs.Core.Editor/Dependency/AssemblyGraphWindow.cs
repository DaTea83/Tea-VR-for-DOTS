...
namespace BovineLabs.Core.Editor.Dependency {
    ...
    public class AssemblyGraphWindow : EditorWindow {
        ...
        private static void Execute() { GetWindow<AssemblyGraphWindow>(false, "Assembly Graph", true); }
        ...
        private void FindAssetThatDependsOn() {
            ...
            for (var i = 0; i < guids.Length; i++) {
                ...
                EditorUtility.DisplayProgressBar("Searching",
                    "AssemblyDefinitionAsset:" + path + " " + i + "/" + guids.Length, (float)i / guids.Length);
                ...
                foreach (var dependency in json.references) {
                    ...
                    if (index != -1) {
                        this.dependencyData[index].Dependencies.Add((
                            AssetDatabase.LoadAssetAtPath<AssemblyDefinitionAsset>(path), path));
                        break;
                    }
        ...
        private void FindAuthoringProblems() {
            ...
            try {
                ...
                for (var i = 0; i < guids.Length; i++) {
                    ...
                    var path = AssetDatabase.GUIDToAssetPath(guid);
                    EditorUtility.DisplayProgressBar("Searching",
                        "AssemblyDefinitionAsset:" + path + " " + i + "/" + guids.Length,
                    ...
                    if (settings.AssembliesToCheck.Any(a => assemblyDefinition.name.EndsWith(a))) {
                        ...
                        badAssemblyGUIDs.Add($"GUID:{AssetDatabase.GUIDFromAssetPath(path)}",
                            (path, assemblyDefinition));
                    }
                }
                ...
                for (var i = 0; i < this.dependencyData.Count; i++) {
                    ...
                    EditorUtility.DisplayProgressBar("Searching",
                        "Check Asset Dependency:" + data.AssetPath + " " + i + "/" + guids.Length,
