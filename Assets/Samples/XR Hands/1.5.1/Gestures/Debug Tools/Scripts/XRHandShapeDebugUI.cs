...
namespace UnityEngine.XR.Hands.Samples.Gestures.DebugTools {
    ...
    public class XRHandShapeDebugUI : MonoBehaviour {
        [SerializeField] [Tooltip("The debug UI that will be used to display the finger states.")]
        ...
        [SerializeField] TextMeshProUGUI SelectedHandShapeName;

        [SerializeField] XRSelectedHandShapeDebugUI m_XRSelectedHandShapeDebugUI;
            ...
            if (m_HandShapeDetected) {
                foreach (var condition in m_HandShape.fingerShapeConditions) {
                    foreach (var shapeCondition in condition.targets) {
                        var xrFingerShapeDebugGraph =
                            m_XRAllFingerShapesDebugUI.xrFingerShapeDebugGraphs[(int)condition.fingerID];
                        var bar = xrFingerShapeDebugGraph.bars[(int)shapeCondition.shapeType];
                        bar.SetTargetAndTolerances(shapeCondition.desired, shapeCondition.upperTolerance,
                            shapeCondition.lowerTolerance);
        ...
        /// </summary>
        public void ClearDetectedHandShape() { handShapeOrPose = null; }
    }
}