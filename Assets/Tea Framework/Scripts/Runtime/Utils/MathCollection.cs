...
namespace TeaFramework
{
    internal static partial class MathCollection
    {
        ...
        public static float SmoothFactor(this float deltaTime, float timeConstant = 0.02f) => 1f - math.exp(-deltaTime / math.max(1e-4f, timeConstant));
        
        public static float3 GetDir(this LocalTransform start, LocalTransform target) => start.Position - target.Position;
        ...
        public static bool GetDistanceAndDot(LocalTransform player, LocalTransform target, out float distanceSqr, out float dot)
        {
        ...
        public static bool GetDistanceAndDot(LocalToWorld player, LocalToWorld target, out float distanceSqr, out float dot)
        {
            var dir = player.GetDir(target);
            distanceSqr = math.lengthsq(dir);
            dot = math.dot(player.Forward, math.normalize(dir));
