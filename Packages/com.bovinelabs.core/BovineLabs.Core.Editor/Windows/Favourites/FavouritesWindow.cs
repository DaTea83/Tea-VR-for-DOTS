...
namespace BovineLabs.Core.Editor.Windows.Favourites {
    ...
    public sealed class FavouritesWindow : BaseObjectWindow<FavouritesItem, FavouritesService, FavouritesPreferences> {
        ...
        /// <inheritdoc/>
        protected override string StylesheetPath =>
            "Packages/com.bovinelabs.core/BovineLabs.Core.Editor/Windows/Favourites/FavouritesWindow.uss";

        /// <inheritdoc/>
        protected override string RootClassName => "favourites-window";

        /// <inheritdoc/>
        protected override GUIContent WindowTitle =>
            new("Favourites", EditorGUIUtility.IconContent("Favorite Icon").image);
        ...
        /// <inheritdoc/>
        protected override void SetupAdditionalFeatures(VisualElement root) { this.SetupDragAndDrop(root); }
        ...
        protected override void BindListItem(VisualElement element, int index) {
            ...
            if (openButton != null) {
                ...
                Action clickHandler = () => { AssetDatabase.OpenAsset(item.GetObject()); };
            ...
            evt.menu.AppendAction("Remove from Favourites",
                _ => {
                    if (this.favouritesService?.ConfirmRemoval == true) {
                        if (!EditorUtility.DisplayDialog("Remove Favourite", $"Remove '{item.Name}' from favourites?",
                                "Remove", "Cancel")) {
                            return;
                        }
                    }
        ...
        protected override void CreateCustomSettingsMenuItems(DropdownMenu menu) {
            ...
            menu.AppendAction("Confirm Removal", _ => { prefs.ConfirmRemoval = !prefs.ConfirmRemoval; }, _ =>
                prefs.ConfirmRemoval
            ...
            menu.AppendAction("Clear All Favourites", _ => {
                if (!EditorUtility.DisplayDialog("Clear All Favourites",
                        "Are you sure you want to remove all favourites? This action cannot be undone.",
                        "Clear All", "Cancel")) {
                    return;
                }
        ...
        private void SetupDragAndDrop(VisualElement root) {
            ...
            root.RegisterCallback<DragLeaveEvent>(_ => { this.dropLabel.style.display = DisplayStyle.None; });
        ...
        private void OnOpenButtonClick(ClickEvent evt) { evt.StopPropagation(); }
