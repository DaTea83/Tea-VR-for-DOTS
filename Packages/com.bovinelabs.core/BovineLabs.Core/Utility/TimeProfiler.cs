...
namespace BovineLabs.Core.Utility {
    ...
    public readonly struct TimeProfiler : IDisposable {
...
#if UNITY_EDITOR
            return logLevel < BLLogger.Level
                ? default
                : new TimeProfiler(ProfilerUnsafeUtility.Timestamp, text, logLevel, 0);
#else
            return default;
#endif
        }

        public static TimeProfiler StartWithMin(FixedString64Bytes text,
            int min,
            LogLevel logLevel = LogLevel.Verbose) {
#if UNITY_EDITOR
            return logLevel < BLLogger.Level
                ? default
                : new TimeProfiler(ProfilerUnsafeUtility.Timestamp, text, logLevel, min);
#else
            return default;
#endif
        }
        ...
        public static TimeProfiler StartString(string text, LogLevel logLevel = LogLevel.Verbose) {
            ...
            return logLevel < BLLogger.Level
                ? default
                : new TimeProfiler(ProfilerUnsafeUtility.Timestamp, text.ToFixedString64NoError(), logLevel, 0);
#else
            return default;
#endif
        }
        ...
        public static TimeProfiler StartStringWithMin(string text, int min, LogLevel logLevel = LogLevel.Verbose) {
            ...
            return logLevel < BLLogger.Level
                ? default
                : new TimeProfiler(ProfilerUnsafeUtility.Timestamp, text.ToFixedString64NoError(), logLevel, min);
#else
            return default;
#endif
        }
