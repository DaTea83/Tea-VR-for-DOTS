...
namespace BovineLabs.Core.Collections {
    ...
    public unsafe struct UnsafeDynamicBuffer<T> : IQueryTypeParameter, IEnumerable<T>, INativeList<T>
        ...
        [NoAlias] private BufferHeader* buffer;
        ...
        public int Length { readonly get => this.buffer->Length; set => this.ResizeUninitialized(value); }

        /// <summary> Gets or sets the number of elements the buffer can hold. </summary>
        public int Capacity {
            readonly get { return this.buffer->Capacity; }
            set {
...
#endif
                BufferHeader.SetCapacity(this.buffer, value, UnsafeUtility.SizeOf<T>(), UnsafeUtility.AlignOf<T>(),
                    BufferHeader.TrashMode.RetainOldData, false,
        ...
        public void EnsureCapacity(int length) {
            BufferHeader.EnsureCapacity(this.buffer, length, UnsafeUtility.SizeOf<T>(), UnsafeUtility.AlignOf<T>(),
                BufferHeader.TrashMode.RetainOldData, false,
                0);
        }
        ...
        /// is not overwritten.
        /// </remarks>
        public void Clear() { this.buffer->Length = 0; }
        ...
        public void Insert(int index, T elem) {
            ...
            UnsafeUtility.MemMove(basePtr + ((index + 1) * elemSize), basePtr + (index * elemSize),
        ...
        public void AddRange(NativeArray<T> newElems) {
            ...
            var basePtr = BufferHeader.GetElementPointer(this.buffer);
            UnsafeUtility.MemCpy(basePtr + ((long)oldLength * elemSize), newElems.GetUnsafeReadOnlyPtr(),
        ...
        public void* GetUnsafePtr() { return BufferHeader.GetElementPointer(this.buffer); }
        ...
        /// <returns> A typed, unsafe pointer to the first element in the buffer. </returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void* GetUnsafeReadOnlyPtr() { return BufferHeader.GetElementPointer(this.buffer); }
        ...
        private static void AssertReinterpretSizesMatch<TU>()
            ...
            if (UnsafeUtility.SizeOf<TU>() != UnsafeUtility.SizeOf<T>()) {
                throw new InvalidOperationException(
                    $"Types {typeof(TU)} and {typeof(T)} are of different sizes; cannot reinterpret");
            }
        }
        ...
        public readonly NativeArray<T> AsNativeArray() {
            var shadow =
                NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<T>(
                    BufferHeader.GetElementPointer(this.buffer), this.Length, Allocator.None);
    ...
    internal sealed class DynamicBufferDebugView<T>
        ...
        public DynamicBufferDebugView(UnsafeDynamicBuffer<T> source) { this.buffer = source; }
