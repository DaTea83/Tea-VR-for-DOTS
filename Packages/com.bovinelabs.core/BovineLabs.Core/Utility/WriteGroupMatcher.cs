...
namespace BovineLabs.Core.Utility {
    ...
    public struct WriteGroupMatcher<T> : IDisposable {
        [ReadOnly] private NativeArray<ComponentType> enableComponentTypes;
        private EntityQueryMask entityQueryMask;

        public WriteGroupMatcher(ref SystemState state) {
            TypeManagerUtil.GetWriteGroupComponents<T>(Allocator.Persistent, out this.enableComponentTypes,
                out var normalComponents);
        ...
        public void Dispose() { this.enableComponentTypes.Dispose(); }
