...
namespace BovineLabs.Core.Keys {
    ...
    public abstract class KSettings<T, TV> : KSettingsBase<T, TV>
        ...
        [SerializeField] private NameValue<TV>[] keys = Array.Empty<NameValue<TV>>();

        public override IEnumerable<NameValue<TV>> Keys => this.keys;

        public TV this[string key] => this.keys.First(k => k.Name == key).Value;

        protected virtual IEnumerable<NameValue<TV>> SetReset() { return Enumerable.Empty<NameValue<TV>>(); }

#if UNITY_EDITOR
        private void Reset() { this.keys = this.SetReset().ToArray(); }
#endif
    }
}