...
namespace BovineLabs.Core.Internal {
    ...
    public static class ArchetypeChunkInternal {
        public static unsafe void SetChangeFilter<T>(this ArchetypeChunk chunk, ref ComponentTypeHandle<T> handle)
            ...
            archetype->Chunks.SetChangeVersion(typeIndexInArchetype, chunk.m_Chunk.ListIndex,
                handle.GlobalSystemVersion);

#if (UNITY_EDITOR || DEVELOPMENT_BUILD) && !DISABLE_ENTITIES_JOURNALING
            if (Hint.Unlikely(chunk.m_EntityComponentStore->m_RecordToJournal != 0)) {
                var ptr = ChunkDataUtility.GetComponentDataWithTypeRW(chunk.m_Chunk, archetype, 0, handle.m_TypeIndex,
                    handle.GlobalSystemVersion,
        ...
        public static unsafe void SetChangeFilter<T>(this ArchetypeChunk chunk,
            ref ComponentTypeHandle<T> handle,
            uint version)
            where T : unmanaged, IComponentData {
            SetChangeFilterCheckWriteAndThrow(handle);
...
#if (UNITY_EDITOR || DEVELOPMENT_BUILD) && !DISABLE_ENTITIES_JOURNALING
            if (Hint.Unlikely(chunk.m_EntityComponentStore->m_RecordToJournal != 0)) {
                chunk.JournalAddRecord(EntitiesJournaling.RecordType.GetBufferRW, handle.m_TypeIndex,
                    handle.m_GlobalSystemVersion);
            }
#endif
        }

        public static unsafe void SetChangeFilter<T>(this ArchetypeChunk chunk,
            ref BufferTypeHandle<T> handle,
            uint version)
            where T : unmanaged, IBufferElementData {
            SetChangeFilterCheckWriteAndThrow(handle);
        ...
        public static unsafe void SetChangeFilter(this ArchetypeChunk chunk, ref DynamicComponentTypeHandle handle) {
            ...
            if (Hint.Unlikely(chunk.m_EntityComponentStore->m_RecordToJournal != 0)) {
                ...
                chunk.JournalAddRecord(EntitiesJournaling.RecordType.GetComponentDataRW, handle.m_TypeIndex,
                    handle.m_GlobalSystemVersion, ptr, byteLen);
            }
#endif
        }
