...
namespace BovineLabs.Core.Editor.Helpers {
    ...
    public static class SerializedHelper {
        public static IEnumerable<SerializedProperty> IterateAllChildren(SerializedObject root,
            bool includeScript,
            bool siblingProperties = false) {
                ...
                if (iterator.propertyPath != "m_Script") {
                    if (iterator.isArray) {
                        ...
                        if (iterator.propertyType !=
                            SerializedPropertyType.ObjectReference) // TODO probably a few more things here
                        {
                            foreach (var child in GetChildren(iterator)) {
