...
namespace BovineLabs.Core.Iterators {
    ...
    public unsafe struct UnsafeEntityDataAccess {
        ...
        public static byte* GetComponentDataPtrRW(ArchetypeChunk archetypeChunk,
        ...
        public static byte* GetChunkComponentDataPtrRW(ArchetypeChunk archetypeChunk,
            ComponentType componentType,
            uint globalSystemVersion) {
            archetypeChunk.m_EntityComponentStore->AssertEntityHasComponent(archetypeChunk.m_Chunk.MetaChunkEntity,
                componentType.TypeIndex);
            return archetypeChunk.m_EntityComponentStore->GetComponentDataWithTypeRW(
                archetypeChunk.m_Chunk.MetaChunkEntity, componentType.TypeIndex,
                globalSystemVersion);
        }
        ...
        public static byte* GetChunkComponentDataPtrRO(ArchetypeChunk archetypeChunk, ComponentType componentType) {
            archetypeChunk.m_EntityComponentStore->AssertEntityHasComponent(archetypeChunk.m_Chunk.MetaChunkEntity,
                componentType.TypeIndex);
            return archetypeChunk.m_EntityComponentStore->GetComponentDataWithTypeRO(
                archetypeChunk.m_Chunk.MetaChunkEntity, componentType.TypeIndex);
        }

        public static UnsafeUntypedDynamicBufferAccessor GetDynamicBufferAccessor(ArchetypeChunk chunk,
            ComponentType componentType,
            uint globalSystemVersion) {
            ...
            var ptr = ChunkDataUtility.GetComponentDataRW(chunk.m_Chunk, archetype, 0, typeIndexInArchetype,
                globalSystemVersion);
        ...
        public static ref readonly v128 GetRequiredEnabledBitsRO(ArchetypeChunk archetypeChunk,
            ComponentType componentType) {
            var archetype = archetypeChunk.m_EntityComponentStore->GetArchetype(archetypeChunk.m_Chunk);
            var indexInTypeArray = ChunkDataUtility.GetIndexInTypeArray(archetype, componentType.TypeIndex);
            ...
            if (Hint.Unlikely(indexInTypeArray == -1)) {
                throw new ArgumentException(
                    $"Required component {componentType.ToFixedString()} not found in archetype.");
            }
#endif

            var ptr = ChunkDataUtility
                .GetEnabledRefRO(archetypeChunk.m_Chunk, archetypeChunk.Archetype.Archetype, indexInTypeArray).Ptr;
        ...
        /// </returns>
        public readonly bool Exists(Entity entity) { return this.access->EntityComponentStore->Exists(entity); }
        ...
        public readonly byte* GetComponentDataPtrRW(Entity entity, ComponentType componentType) {
            return this.HasComponent(entity, componentType)
                ? this.access->EntityComponentStore->GetComponentDataWithTypeRW(entity, componentType.TypeIndex,
                    this.GlobalSystemVersion)
                : null;
        }
        ...
        public readonly UnsafeUntypedDynamicBuffer GetUntypedBufferRO(Entity entity, ComponentType componentType) {
            ...
            var header =
                (BufferHeader*)this.access->EntityComponentStore->GetComponentDataWithTypeRO(entity, typeIndex);

            var internalCapacity = TypeManager.GetTypeInfo(typeIndex).BufferCapacity;
            var typeInfo = TypeManager.GetTypeInfo(typeIndex);

            return new UnsafeUntypedDynamicBuffer(header, internalCapacity, typeInfo.ElementSize,
                UntypedDynamicBuffer.AlignOf);
        }
        ...
        public readonly UnsafeUntypedDynamicBuffer GetUntypedBufferRW(Entity entity, ComponentType componentType) {
            ...
            var header =
                (BufferHeader*)this.access->EntityComponentStore->GetComponentDataWithTypeRW(entity, typeIndex,
                    this.GlobalSystemVersion);
