        ...
        public override void OnActivate(string searchContext, VisualElement rootElement) {
            ...
            foreach (var pair in Preferences) {
                ...
                foreach (var preference in pair.Value) {
                    ...
                    propertyElement.RegisterCallback<GeometryChangedEvent>(e =>
                        StylingUtility.AlignInspectorLabelWidth(propertyElement));
        ...
        private static void CachePreferences() {
            ...
            foreach (var type in TypeCache.GetTypesWithAttribute<T>()) {
                ...
                if (!typeof(IEditorPreference).IsAssignableFrom(type)) {
                    Debug.LogError(
                        $"{Prefix} type `{type.FullName}` must implement `{typeof(IEditorPreference)}` in order to be used as a preference.");
                    continue;
                }
                ...
                if (getOrCreateMethod == null) {
                    Debug.LogError(
                        $"{Prefix} Could not find the `GetOrCreate` method on `{userSettingsType.FullName}` class.");
                    continue;
                }
                ...
                foreach (var attribute in attributes) {
                    var preference =
                        (IEditorPreference)getOrCreateMethod.Invoke(null, new object[] { attribute.SectionName });
