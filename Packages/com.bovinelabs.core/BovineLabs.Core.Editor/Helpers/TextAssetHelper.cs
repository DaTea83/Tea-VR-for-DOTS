...
namespace BovineLabs.Core.Editor.Helpers {
    ...
    public static class TextAssetHelper {
        public static void CreateForProperty(SerializedProperty serializedProperty,
            string defaultName,
        ...
        private static bool TryGetPath(TextAsset data, string defaultName, out string path) {
            path = data == null
                ? EditorUtility.SaveFilePanelInProject($"Save {defaultName}", $"{defaultName}.bytes", "bytes",
