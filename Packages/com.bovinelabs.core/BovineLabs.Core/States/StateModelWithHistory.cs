...
namespace BovineLabs.Core.States {
    ...
    public struct StateModelWithHistory {
        ...
        public StateModelWithHistory(
            ...
            int maxHistorySize) {
            Assert.AreEqual(UnsafeUtility.SizeOf<byte>(),
                TypeManager.GetTypeInfo(stateComponent.TypeIndex).ElementSize);
            Assert.AreEqual(UnsafeUtility.SizeOf<byte>(),
                TypeManager.GetTypeInfo(previousStateComponent.TypeIndex).ElementSize);
            Assert.AreEqual(UnsafeUtility.SizeOf<byte>(),
                TypeManager.GetTypeInfo(historyBackComponent.TypeIndex).ElementSize);
            Assert.AreEqual(UnsafeUtility.SizeOf<byte>(),
                TypeManager.GetTypeInfo(historyForwardComponent.TypeIndex).ElementSize);
        ...
        private struct StateJob : IJobChunk {
            ...
            public void Execute(in ArchetypeChunk chunk,
                ...
                var states =
                    chunk.GetDynamicComponentDataArrayReinterpret<byte>(ref this.StateType,
                        UnsafeUtility.SizeOf<byte>());
                var previousStates =
                    chunk.GetDynamicComponentDataArrayReinterpret<byte>(ref this.PreviousStateType,
                        UnsafeUtility.SizeOf<byte>());
