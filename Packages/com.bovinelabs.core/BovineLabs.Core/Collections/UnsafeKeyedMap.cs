...
namespace BovineLabs.Core.Collections {
    ...
    internal unsafe struct KeyedMapData {
        ...
        [FieldOffset(36)] internal int BucketCapacity;

        [FieldOffset(40)] internal int Length;

        internal static void AllocateHashMap<TValue>(int length,
            int bucketLength,
            AllocatorManager.AllocatorHandle label,
            out KeyedMapData* data)
            where TValue : unmanaged {
            data = (KeyedMapData*)Memory.Unmanaged.Allocate(sizeof(KeyedMapData), UnsafeUtility.AlignOf<KeyedMapData>(),
                label);
            ...
            var totalSize = CalculateDataSize<TValue>(length, bucketLength, out var keyOffset, out var nextOffset,
        ...
        internal static void ReallocateHashMap<TValue>(KeyedMapData* data,
            int newCapacity,
            AllocatorManager.AllocatorHandle label)
            where TValue : unmanaged {
            ...
            var totalSize = CalculateDataSize<TValue>(newCapacity, data->BucketCapacity, out var keyOffset,
        ...
        private static int CalculateDataSize<TValue>(int length,
            int bucketLength,
            out int keyOffset,
            out int nextOffset,
    ...
    internal unsafe struct UnsafeKeyedMapDataDispose {
        [NativeDisableUnsafePtrRestriction] internal KeyedMapData* Buffer;
        ...
        public void Dispose() { KeyedMapData.DeallocateHashMap(this.Buffer, this.AllocatorLabel); }
    }
    ...
    public unsafe struct UnsafeKeyedMap<TValue>
        ...
        public void SetLength(int length) { this.buffer->Length = length; }
        ...
        public int* GetUnsafeKeysPtr() { return (int*)this.buffer->Keys; }

        public TValue* GetUnsafeValuesPtr() { return (TValue*)this.buffer->Values; }
