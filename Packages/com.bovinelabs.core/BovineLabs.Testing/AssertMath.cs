...
namespace BovineLabs.Testing {
    ...
    public static class AssertMath {
        ...
        public static void AreApproximatelyEqual(quaternion expected, quaternion result, float delta) {
            Assert.AreApproximatelyEqual(expected.value.x, result.value.x, delta,
                $"Expected: {result} == {expected}, +-{delta}");
            Assert.AreApproximatelyEqual(expected.value.y, result.value.y, delta,
                $"Expected: {result} == {expected}, +-{delta}");
            Assert.AreApproximatelyEqual(expected.value.z, result.value.z, delta,
                $"Expected: {result} == {expected}, +-{delta}");
            Assert.AreApproximatelyEqual(expected.value.w, result.value.w, delta,
                $"Expected: {result} == {expected}, +-{delta}");
        }
