...
namespace TeaFramework
{
    ...
    public partial struct GrabInteractionISystem : ISystem
    {
        ...
        private static void GetClosestInteractable(DynamicBuffer<GrabberEntitiesIBuffer> buffer, out Entity entity, out float disSqr)
        {
            if (buffer.Length == 0)
            {
                entity = Entity.Null;
        ...
        private static bool SetInteractable(RefRW<GrabberActiveIData> active, RefRW<PhysicsVelocity> vel, 
            RefRW<PhysicsMass> mass, RefRW<PhysicsGravityFactor> gravity,
            DynamicBuffer<InteractGrabIBuffer> iBuffer, Entity interact, Entity entity, in float3 offset)
        {
        ...
        private static bool ReleaseInteractable(RefRW<GrabberActiveIData> active, RefRO<PhysicsMassIData> data,
            RefRW<PhysicsMass> mass, RefRW<PhysicsGravityFactor> gravity,
            DynamicBuffer<InteractGrabIBuffer> iBuffer, Entity entity, out Entity currentInteract)
        {
